...
class CheckMessages(threading.Thread):
    ...
    def messageCB(self,conn,msg):
        ...
        self.message_sender.send_nowait("%s: %s" % (str(received_from), str(msg.getBody())))
    ...
    def StepOn(self):
        try:
            ...
            if self.user_length != len(self.alive_users):
                self.message_sender.send_nowait("***Friends List***")
                for user in self.alive_users:
                    if roster.getName(user) != None:
                        self.message_sender.send_nowait("%s" % roster.getName(user))
